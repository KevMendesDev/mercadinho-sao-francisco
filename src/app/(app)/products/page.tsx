...
export default async function ProductsPage({
  ...
  const totalPages = Math.max(1, Math.ceil(filteredTotal / options.size));
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          [UserRole.ADMIN, UserRole.MANAGER].includes(session.role) ? (
            <ProductCreate key="create" />
          ) : null
        }
      />
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        ...
        />
        <StatCard icon={Tags} label="Categorias" value={totalCategories} />
      </section>
      <div className="card mt-5 overflow-hidden">
            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.id}>
                  ...
                  <td>
                    {product.weight
                      ? Number(product.weight).toLocaleString("pt-BR")
                      : "—"}
                  </td>
                  <td>{product.unit}</td>
                  <td>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-200 px-4 py-3 text-sm">
          <span className="text-zinc-500">
            {filteredTotal} produto(s) • página {currentPage} de {totalPages}
          </span>
          <PageSizeSelect value={options.size} />
          <div className="flex gap-2">
            <Link
              aria-disabled={currentPage <= 1}
              className={`btn-secondary py-2 ${currentPage <= 1 ? "pointer-events-none opacity-40" : ""}`}
              href={urlFor(Math.max(1, currentPage - 1))}
            >
              Anterior
            </Link>
            <Link
              aria-disabled={currentPage >= totalPages}
              className={`btn-secondary py-2 ${currentPage >= totalPages ? "pointer-events-none opacity-40" : ""}`}
              href={urlFor(Math.min(totalPages, currentPage + 1))}
            >
              Próxima
            </Link>
          </div>
