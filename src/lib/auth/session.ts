...
function cookieOptions(maxAge: number) {
  return { secure: process.env.NODE_ENV === "production", path: "/", maxAge };
}
...
export async function createSession(
  data: Omit<SessionData, "csrfTokenHash" | "sessionId">,
): Promise<void> {
  requireAuthSecret();
  const now = new Date();
  ...
  await db
    .getRepository<UserSession>("user_sessions")
    .insert({
...
export async function readSession(): Promise<SessionData | null> {
  ...
  const db = await getDataSource();
  const repository = db.getRepository<UserSession>("user_sessions");
  ...
  await repository.update(record.id, {
    ...
    idleExpiresAt: new Date(
      Math.min(now.getTime() + IDLE_MS, record.absoluteExpiresAt.getTime()),
    ),
  });
  const user = await db
...
export async function requireSession(roles?: UserRole[]): Promise<SessionData> {
  ...
  const branch = await db
    ...
    .findOneBy({ id: session.branchId, active: true });
  const hasAccess =
    user?.role === UserRole.ADMIN ||
    (!!user &&
      (await db
        .getRepository<UserBranch>("user_branches")
        .existsBy({ userId: user.id, branchId: session.branchId })));
  if (!user || !user.active || user.deletedAt || !branch || !hasAccess)
    redirect("/login");
