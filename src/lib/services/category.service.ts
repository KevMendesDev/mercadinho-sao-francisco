...
export async function listCategories() {
  ...
  return db.getRepository<Category>("categories").find({ order: { name: "ASC" } });
}
...
export async function listCategoriesPage(page?: number, size?: number) {
  ...
  const usageRows = categories.length ? await db.getRepository<Product>("products").createQueryBuilder("product")
