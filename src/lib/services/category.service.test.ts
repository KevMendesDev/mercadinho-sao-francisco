...
describe("deleteCategory", () => {
  ...
  it("desvincula produtos excluídos antes de excluir a categoria", async () => {
    ...
    const productRepo = { find: vi.fn().mockResolvedValue([product]), save: vi.fn().mockResolvedValue(product) };
    ...
    expect(productRepo.save).toHaveBeenCalledWith({ ...product, categoryId: null });
