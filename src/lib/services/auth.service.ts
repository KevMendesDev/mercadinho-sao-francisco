...
const DUMMY_PASSWORD_HASH =
  "$2b$12$Qqv5D.bvd21xzlr2IHdCquJ7hdH4eWdlKWDJ1N8BqLrV6xA9yO3gG";
...
export async function authenticate(
  ...
  const user = await db
    ...
    .findOne({ where: { email }, withDeleted: true });
  ...
  const branch = await db
    ...
    .findOneBy({ id: branchId, active: true });
  if (!branch)
    throw new BadRequestError("Filial selecionada não está disponível.");
  if (user.role !== UserRole.ADMIN) {
    const hasAccess = await db
      .getRepository<UserBranch>("user_branches")
      .existsBy({ userId: user.id, branchId });
    if (!hasAccess)
      throw new ForbiddenError("Usuário sem acesso a esta filial.");
  }
  await db
    .getRepository<User>("users")
    .update(user.id, { lastAccessAt: new Date() });
  await createSession({
    userId: user.id,
    name: user.name,
    email: user.email,
