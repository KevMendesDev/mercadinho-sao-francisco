...
export async function authenticate(email: string, password: string, branchId: string): Promise<void> {
  ...
  const user = await db.getRepository<User>("User").findOne({ where: { email }, withDeleted: true });
  ...
  const branch = await db.getRepository<Branch>("Branch").findOneBy({ id: branchId, active: true });
  if (!branch) throw new BadRequestError("Filial selecionada não está disponível.");
  if (user.role !== UserRole.ADMIN) {
    const hasAccess = await db.getRepository<UserBranch>("UserBranch").existsBy({ userId: user.id, branchId });
