...
export function CategoryCreate() {
  ...
  return <><button className="btn-primary" onClick={() => setOpen(true)}><Plus size={18}/>Nova categoria</button>{open && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 p-4"><div className="card w-full max-w-md p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-black">Nova categoria</h2><p className="text-sm text-zinc-500">Cadastre uma categoria para o catálogo.</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Fechar"><X/></button></div><form onSubmit={submit} className="space-y-4"><label><span className="mb-1.5 block text-sm font-bold">Nome *</span><input required autoFocus className="field" value={name} onChange={(event) => setName(event.target.value)}/></label>{error && <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}<div className="flex justify-end gap-2"><button type="button" className="btn-secondary" onClick={() => setOpen(false)}>Cancelar</button><button className="btn-primary" disabled={loading}>{loading ? "Salvando..." : "Salvar categoria"}</button></div></form></div></div>}</>;
}
