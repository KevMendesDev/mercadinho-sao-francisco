...
export function ProductEdit({ product }: { product: EditableProduct }) {
  ...
  return <><button className="btn-secondary px-3 py-2" aria-label={`Editar ${product.name}`} onClick={() => setOpen(true)}><Pencil size={16}/>Editar</button>
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4"><div className="card max-h-[92vh] w-full max-w-2xl overflow-y-auto p-6 md:p-7"><div className="mb-6 flex items-start justify-between gap-4"><div><h2 className="text-xl font-black">Editar produto</h2><p className="text-sm text-zinc-500">Altere os dados do catálogo.</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Fechar" className="grid size-10 shrink-0 place-items-center rounded-lg hover:bg-zinc-100"><X/></button></div>
