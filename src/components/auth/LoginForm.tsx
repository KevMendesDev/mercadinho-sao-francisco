...
export function LoginForm() {
  ...
  useEffect(() => {
    requestJson<{ content: BranchOption[] }>(
      "/api/branches?size=100",
      {},
      "Não foi possível carregar as filiais.",
    )
      .then(({ content }) => setBranches(content))
    ...
    <form onSubmit={submit} className="space-y-5">
      <label className="block">
        ...
        <div className="field-with-icon">
          <Mail className="text-zinc-500" size={19} />
          <input
            name="email"
            type="email"
            required
      ...
      <label className="block">
        ...
        <div className="field-with-icon">
          <Building2 className="text-zinc-500" size={19} />
