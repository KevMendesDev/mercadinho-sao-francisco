...
export function StockEntry({ branchId, canCreateProduct }: { branchId: string; canCreateProduct: boolean }) {
  ...
  async function selectByBarcode(value: string) { const code = value.replace(/\D/g, ""); setBarcode(code); if (!/^\d{8,14}$/.test(code)) { setError("Informe um código de barras válido."); return; } try { const data = await requestJson<{ source?: string; product?: ProductOption }>(`/api/products/lookup/${code}`, {}, "Produto não encontrado."); if (data.product) { addProduct(data.product); return; } if (canCreateProduct) { openCreate(code); return; } setError(data.source === "EXTERNAL_UNAVAILABLE" ? "O catálogo externo está indisponível. Cadastre o produto manualmente ou tente novamente." : "Produto não encontrado no catálogo."); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível consultar o produto."); } }
  ...
  return <><button className="btn-primary" onClick={() => setOpen(true)}><Plus size={18}/>Entrada de estoque</button>
    ...
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4"><div className="card w-full max-w-xl p-6"><div className="mb-5 flex items-start justify-between gap-4"><div><h2 className="text-xl font-black">Nova entrada</h2><p className="text-sm text-zinc-500">Será criado um lote interno para esta validade.</p></div><button type="button" aria-label="Fechar entrada de estoque" onClick={() => setOpen(false)} className="grid size-10 shrink-0 place-items-center rounded-lg hover:bg-zinc-100"><X/></button></div>
