...
export function UserManagement({ initialUsers, branches, filters }: { initialUsers: UserRow[]; branches: Branch[]; filters: { search: string; role: string; status: string } }) {
  ...
  return <>
    <div className="card mb-4 flex flex-wrap items-center gap-3 p-4"><form action="/users" className="flex w-full min-w-0 flex-wrap items-center gap-3 lg:flex-1"><div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-3 text-zinc-500" size={18}/><input name="search" defaultValue={filters.search} className="field pl-10" placeholder="Buscar por nome ou e-mail"/></div><select name="role" defaultValue={filters.role} className="field min-w-0 flex-1 sm:w-auto sm:min-w-40 sm:flex-none"><option value="ALL">Todos os perfis</option><option>ADMIN</option><option>MANAGER</option><option>OPERATOR</option></select><select name="status" defaultValue={filters.status} className="field min-w-0 flex-1 sm:w-auto sm:min-w-36 sm:flex-none"><option value="ALL">Todos status</option><option value="ACTIVE">Ativos</option><option value="INACTIVE">Inativos</option></select><button className="btn-secondary w-full sm:w-auto">Filtrar</button></form><button className="btn-primary w-full sm:w-auto" onClick={newUser}><Plus size={18}/>Novo usuário</button></div>
    ...
    <div className="card overflow-hidden"><div className="table-wrap"><table><thead><tr><th>Nome</th><th className="hidden sm:table-cell">E-mail</th><th>Perfil</th><th className="hidden lg:table-cell">Filiais</th><th>Status</th><th className="hidden xl:table-cell">Último acesso</th><th>Ações</th></tr></thead><tbody>
      {initialUsers.map((user) => <tr key={user.id}><td className="font-bold">{user.name}</td><td className="hidden sm:table-cell">{user.email}</td><td><span className={`badge ${user.role === UserRole.ADMIN ? "bg-amber-100 text-amber-900" : user.role === UserRole.MANAGER ? "bg-blue-50 text-blue-700" : "bg-zinc-100 text-zinc-700"}`}>{user.role}</span></td><td className="hidden lg:table-cell">{user.role === UserRole.ADMIN ? "Todas" : user.branches.join(", ") || "—"}</td><td><StatusBadge active={user.active}/></td><td className="hidden xl:table-cell">{user.lastAccessAt ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(user.lastAccessAt)) : "Nunca"}</td><td><div className="flex gap-2"><button onClick={() => edit(user)} className="grid size-9 place-items-center rounded-lg border border-zinc-200" title="Editar"><Pencil size={16}/></button><button onClick={() => toggleActive(user)} className="grid size-9 place-items-center rounded-lg border border-zinc-200" title={user.active ? "Desativar" : "Reativar"}><Power size={16} className={user.active ? "text-zinc-600" : "text-emerald-600"}/></button></div></td></tr>)}
    ...
    {open && <div className="fixed inset-0 z-50 flex justify-end bg-black/35"><div className="h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl">
      <div className="mb-6 flex items-start justify-between gap-4"><div><h2 className="text-xl font-black">{form.id ? "Editar usuário" : "Novo usuário"}</h2><p className="text-sm text-zinc-500">Controle de acesso por perfil e filial.</p></div><button type="button" aria-label="Fechar formulário de usuário" onClick={() => setOpen(false)} className="grid size-10 shrink-0 place-items-center rounded-lg hover:bg-zinc-100"><X/></button></div>
